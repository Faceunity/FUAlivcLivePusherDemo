...
- (void)setupBottomViews {
    ...
    if (_config.orientation == AlivcLivePushOrientationLandscapeLeft || _config.orientation == AlivcLivePushOrientationLandscapeRight ) {
        self.bottomView.frame = CGRectMake(10,
                                           ...
                                           viewHeight);
        self.bottomView.contentSize = CGSizeMake((self.height / 6)*15, viewHeight);
    ...
    self.waterMarkButton = [self setupButtonWithFrame:(CGRectMake(retractX * 7 + viewWidth * 6, 0, viewWidth, viewHeight)) normalTitle:NSLocalizedString(@"watermark_button", nil) selectTitle:nil normalImg:[UIImage imageNamed:@"alivc_pause"] selectImage:[UIImage imageNamed:@"alivc_pause"] action:@selector(publisherWaterMarkButtonAction:)];
    [self.bottomView addSubview:self.waterMarkButton];
    
    self.removeWaterMarkButton = [self setupButtonWithFrame:(CGRectMake(retractX * 8 + viewWidth * 7, 0, viewWidth, viewHeight)) normalTitle:NSLocalizedString(@"remove_watermark_button", nil) selectTitle:nil normalImg:[UIImage imageNamed:@"alivc_pause"] selectImage:[UIImage imageNamed:@"alivc_pause"] action:@selector(publisherRemoveWaterMarkButtonAction:)];
...
- (void)setupMoreSettingViews {
    ...
    UIButton *confirmBtn = [self setupButtonWithFrame:CGRectMake(self.width - 60, 5, 50, 30) normalTitle:NSLocalizedString(@"确定",nil) selectTitle:NSLocalizedString(@"确定",nil) action:@selector(cancel:)];
    ...
    for (int index = 0; index < labelCount; index++) {
        ...
        textField.frame = CGRectMake(CGRectGetMaxX(nameLabel.frame) + retract,
                                     ...
                                     CGRectGetWidth(self.moreSettingView.frame) - AlivcSizeWidth(70),
                                     30);
