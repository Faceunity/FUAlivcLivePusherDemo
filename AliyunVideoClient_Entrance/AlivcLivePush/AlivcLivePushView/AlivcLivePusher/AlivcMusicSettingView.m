...
- (void)setupMusicInfoModel:(AlivcMusicInfoModel *)model {
    ...
    if ([model.name isEqualToString:NSLocalizedString(@"no_music_edit", nil)]) {
...
- (void)fetchData {
    ...
    AlivcMusicInfoModel *model0 = [[AlivcMusicInfoModel alloc] initWithMusicName:NSLocalizedString(@"no_music_edit", nil) musicPath:nil musicDuation:0.0 isLocal:YES];
    
    NSString *path1 = [[NSBundle mainBundle] pathForResource:@"Axol" ofType:@"mp3"];
    AlivcMusicInfoModel *model1 = [[AlivcMusicInfoModel alloc] initWithMusicName:@"Axol" musicPath:path1 musicDuation:0.0 isLocal:YES];
    
    NSString *path2 = [self getMusicPathWithMusicName:@"Pas de Deux"];
    
    AlivcMusicInfoModel *model2 = [[AlivcMusicInfoModel alloc] initWithMusicName:@"Pas de Deux" musicPath:path2 musicDuation:0.0 isLocal:YES];
    
    NSString *path3 = @"http://docs-aliyun.cn-hangzhou.oss.aliyun-inc.com/assets/attach/51991/cn_zh/1511776743437/JUST%202017.mp3";
    AlivcMusicInfoModel *model3 = [[AlivcMusicInfoModel alloc] initWithMusicName:NSLocalizedString(@"push_internet_music", nil) musicPath:path3 musicDuation:0.0 isLocal:NO];
...
- (void)setupMusicListView {
    ...
    self.musicListView.frame = CGRectMake(0,
                                          ...
                                          CGRectGetHeight(self.frame) / 2+15);
...
- (void)setupMusicVolumeView {
    ...
    volumeView.frame = CGRectMake(0,
                                  ...
                                  self.frame.size.height-AlivcSizeWidth(44));
    [self addSubview:volumeView];
    
    int labelCount = 2;
    CGFloat retract = 10;

    CGFloat labelWidth = AlivcSizeWidth(30);
    if (AlivcScreenWidth > AlivcScreenHeight) {
        retract = 6;
        labelWidth = AlivcSizeHeight(30);
    }
    NSArray *nameArray = @[NSLocalizedString(@"push_accompaniment", nil),NSLocalizedString(@"push_voice", nil)];
    ...
    for (int index = 0; index < labelCount; index++) {
        ...
        slider.frame = CGRectMake(CGRectGetMaxX(nameLabel.frame) + retract,
                                  ...
                                  CGRectGetWidth(volumeView.frame)-AlivcSizeWidth(110),
                                  labelWidth);
        [slider setThumbImage:[UIImage imageNamed:@"smallDots"] forState:UIControlStateNormal];
        slider.minimumTrackTintColor = [AlivcUIConfig shared].kAVCThemeColor;
...
- (void)setupBottomView {
    ...
    CGFloat viewWidth = AlivcSizeWidth(50);
    CGFloat viewHeight = viewWidth/4*3;
    
    if (AlivcScreenWidth > AlivcScreenHeight) {
        viewWidth = 50.0/375*self.bounds.size.width;
        viewHeight = 25.0/375*self.bounds.size.width;
    }
    ...
    self.playButton = [self setupButtonWithFrame:(CGRectMake(retractX * 2 + viewWidth, 0, viewWidth, viewHeight))
                                     normalTitle:NSLocalizedString(@"push_stop", nil)
                                     selectTitle:NSLocalizedString(@"push_start", nil)
    ...
    self.loopButton = [self setupButtonWithFrame:(CGRectMake(retractX * 3 + viewWidth * 2, 0, viewWidth, viewHeight))
                                      normalTitle:NSLocalizedString(@"push_close_loop", nil)
                                      selectTitle:NSLocalizedString(@"push_open_loop", nil)
    ...
    self.muteButton = [self setupButtonWithFrame:(CGRectMake(retractX * 4 + viewWidth * 3, 0, viewWidth, viewHeight))
                                        normalTitle:NSLocalizedString(@"push_open_mute", nil)
                                        selectTitle:NSLocalizedString(@"push_close_mute", nil)
    ...
    self.earBackButton = [self setupButtonWithFrame:(CGRectMake(retractX * 5 + viewWidth * 4, 0, viewWidth, viewHeight))
                                        normalTitle:NSLocalizedString(@"push_close_ears_back", nil)
                                        selectTitle:NSLocalizedString(@"push_open_ears_back", nil)
    ...
    self.denoiseButton = [self setupButtonWithFrame:(CGRectMake(retractX * 6 + viewWidth * 5, 0, viewWidth, viewHeight))
                                        normalTitle:NSLocalizedString(@"push_open_audio_denoise", nil)
                                        selectTitle:NSLocalizedString(@"push_close_audio_denoise", nil)
