...
@property (nonatomic, strong) AlivcPushBeautyDataManager *beautyDataManager;
        ...
        if(!_streamingTimer) {
            ...
            dispatch_source_set_event_handler(_streamingTimer, ^{
                ...
                if(_audioStreamFp) {
                    ...
                    if(nowTime  - _lastAudioPTS >= TEST_EXTERN_PCM_BUFFER_SIZE) {
                        ...
                        if(size > 0) {
                            if(self.isUserMainStream){
                                ...
                                [self.livePusher sendPCMData:pcmData size:(int)size sampleRate:44100 channel:2 pts:nowTime];
                            }
                        }
...
- (void)publisherOnClickBeautyButton{
...
#pragma mark - 美颜设置相关的方法

- (void)showBeautifySettingPanel{
    if(!self.beautifySettingController){
        
        AlivcPushBeautyParamsLevel level = [_beautyDataManager getBeautyLevel];
        NSArray<NSDictionary *> *detailItems = [_beautyDataManager beautyDetailItems];
    ...
    [self presentViewController:self.beautifySettingController animated:YES completion:nil];
}
...
- (void)settingsViewController:(AlivcLiveBeautifySettingsViewController *)viewController didChangeLevel:(NSInteger)level{
    [_beautyDataManager saveBeautyLevel:level];
    
    [viewController updateDetailItems:[_beautyDataManager beautyDetailItems]];
    
    AlivcPushBeautyParams *params = [_beautyDataManager getBeautyParamsOfLevel:level];
    [self setBeautyWithParams:params];
}

- (void)settingsViewController:(AlivcLiveBeautifySettingsViewController *)viewController didChangeValue:(NSDictionary *)info{
    
    [_beautyDataManager saveParamWithInfo:info];
    AlivcPushBeautyParams *params = [_beautyDataManager getBeautyParamsOfLevel:[_beautyDataManager getBeautyLevel]];
    [self setBeautyWithParams:params];
}


- (void)setBeautyWithParams:(AlivcPushBeautyParams *)params{
    [self.livePusher setBeautyWhite:params.beautyWhite];
    [self.livePusher setBeautyBuffing:params.beautyBuffing];
    [self.livePusher setBeautyCheekPink:params.beautyCheekPink];
    [self.livePusher setBeautyRuddy:params.beautyRuddy];
    [self.livePusher setBeautyThinFace:params.beautySlimFace];
    [self.livePusher setBeautyShortenFace:params.beautyShortenFace];
    [self.livePusher setBeautyBigEye:params.beautyBigEye];
}
