...
char const *_DLKeyboardViewFrameObserverKey;

@interface _DLKeyboardViewFrameObserver : NSObject
...
@implementation _DLKeyboardViewFrameObserver {
...
- (void)addToKeyboardView:(UIView *)keyboardView {
    ...
    if (_keyboardView) {
        ...
        objc_setAssociatedObject(_keyboardView, &_DLKeyboardViewFrameObserverKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    _keyboardView = keyboardView;
    if (keyboardView) {
        [self addFrameObserver];
    }
    objc_setAssociatedObject(keyboardView, &_DLKeyboardViewFrameObserverKey, self, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
+ (instancetype)observerForView:(UIView *)keyboardView {
    ...
    return objc_getAssociatedObject(keyboardView, &_DLKeyboardViewFrameObserverKey);
}
...
@implementation AlivcKeyboardManager {
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"DLKeyboardManager init error" reason:@"defaultManager' to get instance." userInfo:nil];
    return [super init];
}
...
- (void)_initFrameObserver {
    ...
    _DLKeyboardViewFrameObserver *observer = [_DLKeyboardViewFrameObserver observerForView:keyboardView];
    if (!observer) {
        observer = [_DLKeyboardViewFrameObserver new];
...
+ (instancetype)defaultManager {
    static AlivcKeyboardManager *dmg = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (![UIApplication isAppExtension]) {
            dmg = [[self alloc] _init];
        }
    });
    return dmg;
}
...
- (void)_notifyAllObservers {
    ...
    if (!CGRectEqualToRect(trans.toFrame, _fromFrame)) {
        for (id<AVCKeyboardObserver> observer in _observers.copy) {
